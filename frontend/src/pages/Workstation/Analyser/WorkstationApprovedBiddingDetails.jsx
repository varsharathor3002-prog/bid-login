...
export default function WorkstationApprovedBiddingDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (bid) return;
    fetch(`${API_BASE}/workstation-bids/${id}/`)
      .then(async (response) => {
  ...
  const downloadPdf = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/workstation-bids/${id}/generate-docs/`, {
        method: "POST",
