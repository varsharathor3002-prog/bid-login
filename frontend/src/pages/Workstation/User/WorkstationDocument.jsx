...
export default function WorkstationDocument({ bidData, onSuccess, onBack, submitLabel = "Forward to Analyser", analyserMode = false, docOptions = null }) {
  ...
  const toggleGeneralDocSelection = (docId) => {
    setSelectedGeneralDocs((prev) => ({ ...prev, [docId]: !prev[docId] }));
  };
  ...
  const handleViewCertificate = async (docId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/workstation-bids/${bidData.bid_id}/generate-docs/`,
        {
          method: "POST",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/workstation-bids/${bidData?.bid_id}/update-docs/`,
        {
          method: "POST",
          body: formData,
        }
                    ...
                    {activeDocs.filter((doc) => selectedGeneralDocs[doc.id]).map((doc) => (
                      <div key={doc.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 8, padding: "7px 12px", gap: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: "#9a3412", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {doc.label}
                          </span>
                        </div>
                        <button type="button" className="remove-icon-btn" onClick={() => removeGeneralDocSelection(doc.id)}>✕</button>
                      </div>
                    ))}
                    ...
                    {!analyserMode && specialDoc && (
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 8, padding: "7px 12px", gap: 10 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                          <span style={{ fontSize: 10, color: "#7c3aed", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {specialDoc.name}
                          </span>
                          <button type="button" className="remove-icon-btn" onClick={() => setSpecialDoc(null)}>✕</button>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="modal-box">
            <div className="modal-header-orange">
              ...
              <button className="close-btn close-btn-orange" onClick={() => setModal(null)}>✕</button>
            </div>
            ...
            <div style={{ maxHeight: 420, overflowY: "auto" }}>
              <div style={{ padding: "10px 20px", borderBottom: "1px solid #f1f5f9", background: "#fffbeb", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button type="button" onClick={clearGeneralDocsSelection} style={{ border: "1px solid #cbd5e1", background: "#fff", color: "#475569", borderRadius: 8, padding: "5px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                    Clear
                  </button>
                </div>
              </div>
                ...
                return (
                  <div key={doc.id} className="doc-row">
                    <label style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, cursor: "pointer", minWidth: 0 }}>
                      ...
                      />
                      <span style={{ fontSize: 12, fontWeight: 600, color: isSelected ? "#9a3412" : "#334155", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {doc.label}
                      </span>
                    </label>
                    {isSelected && (
            ...
            <div className="done-footer">
              <button className="done-btn" onClick={() => setModal(null)}>Save Selection</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="modal-box">
            <div className="modal-header-orange" style={{ background: "linear-gradient(135deg, #f5f3ff, #ede9fe)", borderBottomColor: "#ddd6fe" }}>
              ...
              <button className="close-btn close-btn-orange" style={{ background: "#ddd6fe", color: "#5b21b6" }} onClick={() => setModal(null)}>✕</button>
            </div>
              ...
              {specialDoc && (
                <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "10px 14px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: "#15803d", maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {specialDoc.name}
                    </span>
                  </div>
                  <button type="button" onClick={() => setSpecialDoc(null)} className="remove-icon-btn">✕</button>
                </div>
              )}
            </div>

            <div className="done-footer">
              <button className="done-btn" style={{ background: "linear-gradient(135deg, #8b5cf6, #7c3aed)" }} onClick={() => setModal(null)}>Done</button>
            </div>
          </div>
        </div>
      )}
