...
export default function CreateWorkstationBid() {
  ...
  useEffect(() => {
    localStorage.setItem("workstation_bid_data", JSON.stringify(allData));
  }, [allData]);
  ...
  useEffect(() => {
    ...
    const handleBack = (e) => {
      ...
      if (step > 1) {
        ...
        setStep(newStep);
        localStorage.setItem("workstation_bid_step", newStep);
...
function Step1Form({ onNext, savedData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/workstation-bids/create/`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="w-full max-w-[600px] bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="flex items-center gap-3 mb-5">
        <h5 className="text-lg font-semibold text-gray-800">Create Workstation Bid</h5>
      </div>
