...
export default function TonerDocument({ bidData, onSuccess }) {
  ...
  const [selectedYieldDocs, setSelectedYieldDocs] = useState({});
  ...
  const selectedYieldDocIds = YIELD_STANDARD_DOCS.filter((doc) => selectedYieldDocs[doc.id]).map((doc) => doc.id);
  const selectedYieldDocLabels = YIELD_STANDARD_DOCS.filter((doc) => selectedYieldDocs[doc.id]).map((doc) => doc.label);
  const selectedYieldDocsCount = selectedYieldDocIds.length;
  ...
  const toggleYieldDocSelection = (docId) => {
    setSelectedYieldDocs((prev) => ({
      ...prev,
      [docId]: !prev[docId],
    }));
  };

  const removeYieldDocSelection = (docId) => {
    setSelectedYieldDocs((prev) => {
      const updated = { ...prev };
      delete updated[docId];
      return updated;
  ...
  const selectAllYieldDocs = () => {
    const allSelected = {};
    YIELD_STANDARD_DOCS.forEach((doc) => {
      allSelected[doc.id] = true;
    });
    setSelectedYieldDocs(allSelected);
  };

  const clearYieldDocsSelection = () => {
    setSelectedYieldDocs({});
  };
  ...
  const handleViewCertificate = async (docId) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/toner-bids/${bidData.bid_id}/generate-docs/`, {
        method: "POST",
      ...
      if (data.pdf_url) {
        ...
        if (!fileResponse.ok) throw new Error("No PDF URL received");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("selected_yield_standard_docs", JSON.stringify(selectedYieldDocIds));
      formData.append("selected_yield_standard_doc_labels", JSON.stringify(selectedYieldDocLabels));
      ...
      const response = await fetch(`${API_BASE}/toner-bids/${bidData?.bid_id}/update-docs/`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <>
        ...
        <div className="doc-card">
          ...
          <div style={{ padding: "24px" }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {(selectedGeneralDocsCount > 0 || selectedYieldDocsCount > 0 || specialDoc) && (
                <div
                  style={{
                    marginBottom: 20,
                    padding: "14px 16px",
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    {YIELD_STANDARD_DOCS.filter((doc) => selectedYieldDocs[doc.id]).map((doc) => (
                      <div
                        key={doc.id}
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        <button
                          type="button"
                          className="remove-icon-btn"
                          onClick={() => removeYieldDocSelection(doc.id)}
      ...
      {modal === "yield" && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="modal-box">
            <div className="modal-header-orange" style={{ background: "linear-gradient(135deg, #ecfdf5, #d1fae5)", borderBottomColor: "#a7f3d0" }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#047857" }}>Yield Standard Certificates</div>
                <div style={{ fontSize: 11, color: "#059669", marginTop: 2 }}>Select the certificates to send. Use View to preview.</div>
              </div>
              <button className="close-btn close-btn-orange" style={{ background: "#a7f3d0", color: "#047857" }} onClick={() => setModal(null)}>
                ✕
              </button>
            </div>
            ...
            <div style={{ maxHeight: 420, overflowY: "auto" }}>
              ...
              >
                <div style={{ fontSize: 11, fontWeight: 700, color: "#047857" }}>
                  {selectedYieldDocsCount} of {YIELD_STANDARD_DOCS.length} selected
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    type="button"
                    onClick={selectAllYieldDocs}
                    style={{
                  ...
                  <button
                    type="button"
                    onClick={clearYieldDocsSelection}
                    style={{
                      border: "1px solid #cbd5e1",
                      background: "#fff",
                      color: "#475569",
              ...
              {YIELD_STANDARD_DOCS.map((doc) => {
                const isSelected = !!selectedYieldDocs[doc.id];
                  ...
                  <div key={doc.id} className="doc-row">
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => toggleYieldDocSelection(doc.id)}
            ...
            <div className="done-footer">
              <button className="done-btn" style={{ background: "linear-gradient(135deg, #10b981, #059669)" }} onClick={() => setModal(null)}>
                Save Selection
              </button>
            </div>
          </div>
