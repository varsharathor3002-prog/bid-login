...
const SpecField = ({ label, name, form, handleChange, options, isTextArea = false, optional = false }) => (
  <div className="col-span-1">
    ...
    {isTextArea ? (
      ...
      <select
        ...
        value={form?.[name] || ""}
        onChange={handleChange}
        className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
      >
...
function GeneralDocsViewPopup({ form }) {
  ...
  const getGeneratedPdfUrl = async (docId) => {
    ...
    const response = await fetch(`${API_BASE}/toner-bids/${bidId}/generate-docs/`, {
      method: "POST",
...
function OnDemandDocView({ form, docType, label, hint, icon }) {
  ...
  const getGeneratedPdfUrl = async () => {
    if (!bidId) throw new Error("Bid ID not found.");
    // Same to same as AIO's OnDemandDocView: send the form's current
...
export default function TonerBidApproval() {
  ...
  const fetchBids = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/toner-bids/list/?status=${activeTab}&role=admin`);
      if (!res.ok) throw new Error(`Failed with status ${res.status}`);
      const data = await res.json();
      setBids(sortBids(Array.isArray(data) ? data : data.bids || []));
    } catch (error) {
  ...
  const fetchReAnalyzeCount = async () => {
    try {
      const res = await fetch(`${API_BASE}/toner-bids/list/?status=re-analyze&role=admin`);
  ...
  const openModal = (bid) => {
    const formattedBid = { ...bid, total_price: bid.status === "approved" ? bid.total_price : "" };
  ...
  const handleAction = async (action) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/toner-bids/${form.id}/admin-review/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...form,
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl max-h-[calc(100vh-2rem)] overflow-y-auto">
            <div className="flex justify-between items-center px-6 py-4 border-b bg-gray-50 rounded-t-xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-800">Review & Update Toner</h2>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 [&_label]:font-semibold [&_label]:text-slate-800 [&_input]:border-slate-500 [&_input]:bg-slate-50 [&_input]:text-slate-950 [&_select]:border-slate-500 [&_select]:bg-slate-50 [&_select]:text-slate-950 [&_textarea]:border-slate-500 [&_textarea]:bg-slate-50 [&_textarea]:text-slate-950 [&_input::placeholder]:text-slate-600 [&_textarea::placeholder]:text-slate-600">
                ...
                <SpecField label="Warranty Type" name="warranty_type" options={SPEC_OPTIONS.warranty_type} form={form} handleChange={handleChange} />
                <SpecField label="Refillable" name="refillable" options={SPEC_OPTIONS.refillable} form={form} handleChange={handleChange} />
                <SpecField label="Quantity per Pack" name="qty_per_pack" options={SPEC_OPTIONS.qty_per_pack} form={form} handleChange={handleChange} />
                <SpecField label="HSN Code" name="hsn_code" form={form} handleChange={handleChange} />
                <SpecField label="Compliance" name="compliance" options={SPEC_OPTIONS.compliance} optional form={form} handleChange={handleChange} />
                <SpecField label="Replacement Policy" name="replacement_policy" options={SPEC_OPTIONS.replacement_policy} form={form} handleChange={handleChange} />
