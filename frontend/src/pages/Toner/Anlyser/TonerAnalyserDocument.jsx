...
// Same to same as Desktop's Analyserdocument.jsx (STEP 2/2) — only Warranty,
// Technical Compliance and Data Sheet are generated/selected here. Make in
// India stays Admin-only (see TonerBidApproval.jsx's OnDemandDocView), matching
// Desktop where MakeInIndiaView only exists on the Admin side.
const ANALYSER_DOCS = [
...
export default function TonerAnalyserDocument() {
  ...
  const handleViewDocument = async (doc) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/toner-bids/${bidId}/generate-docs/`, {
        ...
        body: JSON.stringify({ doc_type: doc.id }),
      });
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/toner-bids/${bidId}/review/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...bidData,
          status: "analyzed",
          analyser_username: analyserUsername,
          analyser_note: bidData.analyser_note || "",
          verified_fields: bidData.verified_fields || [],
        }),
      });
      ...
      setSuccess(true);
      setTimeout(() => navigate("/analyser-dashboard/toner"), 1400);
    } catch (err) {
