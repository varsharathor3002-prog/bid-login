...
// Same to same as Desktop's ApprovedBiddingDetails.jsx — the "Approved
// Details For Bidding" document view that opens from the Analyser's Approved
// tab / Transfer Catalogue to GeM tab. AIO's own version of this route used
// to just reopen the full bid-analysis form (AioBidDetailView) instead of
// this dedicated one-page document, which is why it looked different from
// Desktop's.
const API_BASE = import.meta.env.VITE_API_URL;

export default function AioApprovedBiddingDetails() {
  const { id } = useParams();
  ...
  const downloadPdf = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/aio-bids/${id}/generate-docs/`, {
        method: "POST",
