...
const ANALYSER_DOCS = [
  {
    id: "warranty",
    label: "WARRANTY",
    fields: ["onsite_warranty", "extended_warranty", "qty", "organization", "bid_no"],
  },
  {
...
export default function AnalyserPrinterDocument() {
  const { state } = useLocation();
  const navigate = useNavigate();
  const bidData = state?.bidData || {};
  const isMultifunction = String(bidData?.printer_type || "").toLowerCase().includes("multifunction");
  ...
  const handleViewDocument = async (doc) => {
    ...
    try {
      ...
      doc.fields
        .filter((key) => isMultifunction || !MULTIFUNCTION_ONLY_FIELDS.has(key))
        .forEach((key) => {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      formData.append("analyser_username", localStorage.getItem("analyser_username") || localStorage.getItem("username") || "Analyser");
      ...
      const res = await fetch(`${API_BASE}/printer-bids/${bidId}/update-docs/`, {
        method: "POST",
        body: formData,
      });
