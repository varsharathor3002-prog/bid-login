...
import PrinterBidSummary from "./PrinterBidSummary";
import PrinterDocument from "./PrinterDocument";
...
const clearBidStorage = (stepKey, dataKey) => {
  localStorage.removeItem(stepKey);
  localStorage.removeItem(dataKey);
};

export default function CreatePrinterBid({ productMode = "printer" }) {
  const storagePrefix = productMode === "multifunction" ? "multifunction_printer" : "printer";
  const stepStorageKey = `${storagePrefix}_bid_step`;
  const dataStorageKey = `${storagePrefix}_bid_data`;
  const [step, setStep] = useState(() => {
    return Number(localStorage.getItem(stepStorageKey)) || 1;
  });
  ...
  useEffect(() => {
    localStorage.setItem(dataStorageKey, JSON.stringify(allData));
  }, [allData, dataStorageKey]);
  ...
  useEffect(() => {
    const handleBack = (e) => {
      ...
      if (step > 1) {
        ...
        clearBidStorage(stepStorageKey, dataStorageKey);
        window.history.back();
      }
    };
  ...
  }, [step, stepStorageKey, dataStorageKey]);
...
function Step1Form({ onNext, savedData, productMode = "printer" }) {
  const printerType = productMode === "multifunction" ? "Multifunction Printer" : "Printer";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (savedData?.bid_id) {
        onNext({ ...form, bid_id: savedData.bid_id, printer_type: printerType });
        return;
      }
      ...
      const response = await fetch(`${API_BASE}/printer-bids/create/`, {
        method: "POST",
        body: formData,
      });
      ...
      onNext({ ...form, bid_id: data.bid_id, printer_type: printerType });
    } catch (err) {
      console.error(err);
    ...
    <div className="w-full max-w-[600px] bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      <div className="flex items-center gap-3 mb-5">
        <h5 className="text-lg font-semibold text-gray-800">
          {productMode === "multifunction" ? "Create Multifunction Printer Bid" : "Create Printer Bid"}
        </h5>
      </div>
