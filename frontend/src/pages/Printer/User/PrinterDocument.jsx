...
export default function PrinterDocument({ bidData, onSuccess, onBack }) {
  ...
  const selectedGeneralDocIds = GENERAL_DOCS.filter(
    (doc) => selectedGeneralDocs[doc.id]
  ).map((doc) => doc.id);
  const selectedGeneralDocLabels = GENERAL_DOCS.filter(
    (doc) => selectedGeneralDocs[doc.id]
  ).map((doc) => doc.label);
  ...
  const handleSpecialFile = (e) => {
    if (e.target.files?.[0]) {
      setSpecialDoc(e.target.files[0]);
    }
  };
  ...
  const handleViewCertificate = async (docId) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/printer-bids/${bidData.bid_id}/generate-docs/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ doc_type: docId, model_number: bidData?.model_number || "" }),
      });
      ...
      console.error(err);
      setError(err.message || "Unable to generate the certificate. Please try again.");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append(
        ...
        JSON.stringify(selectedGeneralDocLabels)
      );
      formData.append("model_number", bidData?.model_number || "");

      const response = await fetch(
        `${API_BASE}/printer-bids/${bidData?.bid_id}/update-docs/`,
        {
          method: "POST",
          body: formData,
        }
          ...
          <div className="modal-box">
            <div className="modal-header-orange">
              <div>
                ...
                <div style={{ fontSize: 11, color: "#c2410c", marginTop: 2 }}>Select the documents to submit.</div>
              </div>
              ...
              {GENERAL_DOCS.map((doc) => {
                ...
                return (
                  <div key={doc.id} className="doc-row">
                    ...
                    <button
                      type="button"
                      onClick={() => handleViewCertificate(doc.id)}
