...
def generate_certificates(request, bid_id):
    ...
    model_number = (bid.model_number or "").strip()
    ...
    def _force_tender_no_date(page):
        ...
        tender_text = f"Tender No: {bid_no if bid_no else ''} Dated: {bid_date_formatted if bid_date_formatted else ''}"
    ...
    def _remove_to_whomsoever_line(page):
        ...
        if not page_already_has_to_block:
            ...
            # ✅ v20: Pura block bold (hebo) — professional letterhead jaisa
            # look, "To," ke alawa baki lines bhi ab bold hain
            cur_y = insert_y
    ...
    try:
        ...
        for page in new_doc:
            ...
            if doc_type in ["bidder_financial", "ipv6", "preloaded_os"]:
                    ...
                    if table_start_idx is not None:
                        ...
                        if len(address_lines) >= 3 and full_address:
                            ...
                            page.insert_textbox(
                                ...
                                full_address,
                                fontsize=10, fontname="hebo", color=(0, 0, 0), align=0,
                            )
            ...
            page_has_tender_no = "Tender No:" in page_text_raw or "Tender No :" in page_text_raw
            ...
            tender_idx = None
            for i in range(to_idx + 1, len(all_lines)):
                if "Tender" in all_lines[i][1] or "tender" in all_lines[i][1]:
                    tender_idx = i
                    break
            ...
            if full_address:
                ...
                for i in range(addr_start_idx, len(all_lines)):
                    ...
                    if tender_idx and i >= tender_idx:
                        break
                    if any(kw in text for kw in ["Subject", "Dear", "Tender"]):
                        break
                ...
                if addr_erase and write_x is not None:
                    ...
                    page.insert_textbox(
                        addr_rect,
                        full_address,
                        fontsize=11, fontname="hebo", color=(0, 0, 0), align=0,
                    )
...
#  *** FIXED: ***
#  - Saves special doc AND all general docs sent from frontend
#    (frontend sends flags like general_doc_<id>=true, but the
#     actual files for those certs are SERVER-GENERATED PDFs,
#     not separate uploads — so we only persist what's actually
#     uploaded: atc_special_document).
#  - ALWAYS sets bid.status = "complete" (so list_desktop_bids,
#    which filters status__in=["complete","configured"], can see it)
#    and bid.review_status = "pending" (so Analyser's "Pending" tab
#    picks it up), REGARDLESS of what `status` value the frontend
#    sends ("pending" etc).
#  This fixes: after completing Step 3, the bid was not appearing
#  in the Analyser dashboard because status stayed "configured"/
#  was incorrectly overwritten to "pending" (which list_desktop_bids
#  does not match against `status`).
# ══════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════
#  STEP 3 — FINAL DOCUMENT SUBMIT (ANALYSER'S FINAL STEP)
# ══════════════════════════════════════════════════════════
@csrf_exempt
@require_http_methods(["POST"])
@csrf_exempt
...
GEM_SECTIONS = [
    ...
    {"title": "MOTHERBOARD", "fields": [
        "Expansion Slots (PCIe x 1)", "Expansion Slots (PCIe x 4)", "Expansion Slots (PCIe x 16)",
        "Expansion Slots (M Dot 2) for SSD", "Expansion Slots (M Dot 2) for WiFi", "Trusted Platform Module"
    ]},
    {"title": "GRAPHICS", "fields": [
        "Graphics Type", "Graphic Card Make and Model - Must declare",
        "Size of Memory in Case of Dedicated Graphic Card(GB)"
    ]},
    {"title": "OPERATING SYSTEM", "fields": [
        "Factory Pre-loaded Operating System by DesktopOEM", "Recovery Media for OS"
    ]},
    {"title": "MEMORY (RAM)", "fields": [
        "Type of RAM",
    ...
    {"title": "STORAGE", "fields": [
        "Type of Storage Installed with the System", "SSD - Storage Capacity (in GB)", "HDD - Storage Capacity (in GB)"
    ]},
    {"title": "BAYS AVAILABILITY", "fields": [
    ...
    {"title": "CABINET", "fields": [
        "Cabinet Form Factor", "Bays for Optical Drive", "Optical Drive", "Audio Interface Type"
    ]},
    ...
    {"title": "PORTS", "fields": [
        ...
        "Number of USB Ports Type C", "Number of VGA Ports", "Number of HDMI Ports", "Number of DP Ports"
    ]},
    {"title": "Monitor", "fields": [
        "Availibility of Monitor", "Panel Type", "Display Technology", "Screen Size (in CMs)",
        "Maximum Resolution (Pixels)", "Image Aspect Ratio", "Brightness (in Nits)", "Refresh Rate (in Hz)",
        "Monitor Port", "Integrated Webcam with Mic", "Power Supply for Monitor", "Speaker"
    ]},
    ...
    {"title": "OPERATING CONDITION", "fields": [
        "Minimum Operating Temperature (in Degree Celsius)",
        "Maximum Operating Temperature (in DegreeCelsius)",
...
FIELD_ALIASES = {
    ...
    "RAM Size (Memory Card/Module) (in GB) (Capacity tobe installed in the System)": [
        ...
        "RAM Size (Memory Card/Module) (in GB) (Capacity to",
    ],
    ...
    "Number of DIMM Slots Populated with MemoryCard/Module": [
        ...
        "Number of DIMM Slots Populated with Memory",
    ],
    ...
    "Number of Internal Bay Populated, Size 2 Point 5Inch": [
        ...
        "Number of Internal Bay Populated, Size 2 Point 5",
    ],
    ...
    "Number of Internal Bay Populated, Size 3 Point 5inch": [
        ...
        "Number of Internal Bay Populated, Size 3 Point 5",
    ],
    ...
    "Maximum Operating Temperature (in DegreeCelsius)": [
        ...
        "Maximum Operating Temperature (in Degree",
    ],
...
def _fix_joined_words(text):
    ...
    replacements = {
        ...
        "DesktopOEM": "Desktop OEM",
        "MemoryCard/Module": "Memory Card/Module",
        "tobe": "to be",
        "5Inch": "5 Inch",
        "5inch": "5 inch",
...
def _clean_line(line):
    ...
    if not line:
        return ""
    if re.search(r"\b\d{1,2}/\d{1,2}/\d{2,4},\s*\d{1,2}:\d{2}\s*[AP]M\b", line, re.IGNORECASE):
        return ""
    if "Government e Marketplace" in line:
        return ""
    if "mkp.gem.gov.in" in line:
        return ""
    if line.startswith("http://") or line.startswith("https://"):
        return ""
    if re.fullmatch(r"\d+/\d+", line):
        return ""
    if line.strip() in {"Product Compare", "Product History", "Product Compare Product History1", "Product Compare Product History 1"}:
        return ""
...
def _select_actual_block(pages):
    ...
    for i, lines in enumerate(page_lines):
        joined = " ".join(lines)
        if (
            re.search(r"\bPROCESSOR\b", joined, re.IGNORECASE)
            and re.search(r"\bMOTHERBOARD\b", joined, re.IGNORECASE)
            and re.search(r"Processor\s+Number", joined, re.IGNORECASE)
        ):
            start_idx = i
            break
    ...
    for lines in page_lines[start_idx:]:
        for line in lines:
            if line.upper() == "PROCESSOR":
                started = True
            if not started:
                continue
            if line.upper() in {"CERTIFICATION", "WEB INFO"}:
                return " ".join(parts)
            parts.append(line)
...
def _find_boundaries(block):
    ...
    for canonical, alias in alias_items:
        for m in re.finditer(_label_regex(alias), block, flags=re.IGNORECASE):
            if canonical == "Optical Drive" and block[max(0, m.start() - 3):m.start()].lower() == "no ":
                continue
            candidates.append({
                "start": m.start(),
                "end": m.end(),
...
def _remove_edge_section_titles(value):
    ...
    titles = [
        "MOTHERBOARD", "GRAPHICS", "OPERATING SYSTEM", "MEMORY (RAM)", "STORAGE",
        "BAYS AVAILABILITY", "CABINET", "CONNECTIVITY", "PORTS", "Monitor",
        "INPUT DEVICES", "POWER", "OPERATING CONDITION", "WARRANTY"
    ]
    for _ in range(3):
        for title in titles:
...
def _valid_model_no(model):
    ...
    if not model or model in banned:
        return False
    if len(model) < 8:
        return False
    if model.count("-") < 2:
...
def _guess_category(text):
    ...
    if "toner" in low or "cartridge" in low:
        return "Toner"
    if "printer" in low:
        return "Printer"
    if "desktop" in low or "computer" in low:
        return "Desktop"
    if "aio" in low or "all in one" in low:
        return "AIO"
    return ""
...
def extract_catalogue_pdf(request):
    try:
        ...
        if not extra_specs.get("HDD - Storage Capacity (in GB)"):
            ...
            if hdd_val:
                extra_specs["HDD - Storage Capacity (in GB)"] = hdd_val
        ...
        if not model_no:
            return JsonResponse({"error": "Model No. PDF se extract nahi hua."}, status=400)
...
def list_catalogue_products(request):
    try:
        ...
        if search:
            qs = qs.filter(model_no__icontains=search)
...
def create_catalogue_product(request):
    ...
    try:
        ...
        if not model_no:
            return JsonResponse({"error": "Model No. is required"}, status=400)

        if model_no in {"MEITY", "DESKTOP", "COMPUTER"}:
            return JsonResponse({"error": "Invalid model no. Please upload correct PDF."}, status=400)

        if CatalogueProduct.objects.filter(model_no__iexact=model_no).exists():
            return JsonResponse({"error": "A product with this model no. already exists"}, status=400)

        try:
            extra_specs = json.loads(data.get("extra_specs", "{}"))
        except Exception:
            extra_specs = {}
...
def update_catalogue_product(request, product_id):
    ...
    try:
        ...
        if model_no:
            if model_no in {"MEITY", "DESKTOP", "COMPUTER"}:
                return JsonResponse({"error": "Invalid model no."}, status=400)

            duplicate = CatalogueProduct.objects.filter(model_no__iexact=model_no).exclude(id=product.id).exists()
            if model_no != product.model_no and duplicate:
                return JsonResponse({"error": "Another product with this model no. already exists"}, status=400)
...
def _bid_data(bid, request, status_label=None):
    return {
        ...
        "motherboard_type": bid.motherboard_type or "",
        "motherboard": bid.motherboard or "",
        "motherboard_price": bid.motherboard_price or 0,
        "motherboard_descp": bid.motherboard_descp or "",
        "motherboard_descp_price": bid.motherboard_descp_price or 0,

        "software1": bid.software1 or "",
        "software1_price": bid.software1_price or 0,
        "gp": bid.gp or "",
        "gp_price": bid.gp_price or 0,
...
def _get_model_number_from_data(data):
    """
    Frontend alag-alag keys bhej raha hai:
    model_number / model / selected_model / matched_model / model_no.
    Kabhi selected_model object bhi aa sakta hai. Ye helper sab handle karega.
    """
    if data is None:
        return ""
    ...
    model = (
        ...
        or data.get("matched_model")
        or data.get("model_no")
        or data.get("modelNo")
        or ""
    )
    ...
    if isinstance(model, dict):
        model = (
            ...
            or model.get("model_no")
            or model.get("modelNo")
            or model.get("model")
            or ""
        )
...
# ══════════════════════════════════════════════════════════
#  CATALOGUE MATCH API — FINAL UPDATED
# ══════════════════════════════════════════════════════════
# Final logic:
# 1) Backend internally 12 fields check karega.
# 2) SSD2 ignore hoga; SSD matching sirf SSD1 se hogi.
# 3) Frontend ko field count/details nahi bheje jayenge.
# 4) Agar 12/12 match mila to sirf 1 model return hoga.
# 5) Agar 12/12 match nahi mila to match=None return hoga, frontend create option dikhayega.
# 6) Monitor: 19 inch / 19.5 inch / range tolerance accept hoga.
# 7) Cabinet: SFF == Small Form Factor accept hoga.
# 8) Keyboard & Mouse Wired == USB Wired accept hoga.
# 9) Motherboard: dropdown me mentioned features ka 50%+ match pass hoga
#    (MOTHERBOARD section + PORTS section + CONNECTIVITY section combined).
# ══════════════════════════════════════════════════════════
...
def _match_clean(value):
    ...
    v = v.replace("windows 11 professional", "windows 11 pro")
    v = v.replace("window 11 professional", "windows 11 pro")
    v = v.replace("win 11 professional", "windows 11 pro")
    v = v.replace("windows 11 home", "windows 11 home")
    v = v.replace("window 11 home", "windows 11 home")
    v = v.replace("win11", "windows 11")

    v = v.replace("solid state drive", "ssd")
    v = v.replace("hard disk drive", "hdd")
    v = v.replace("wi fi", "wifi")
    v = v.replace("wi-fi", "wifi")
    v = v.replace("bluetooth", "bt")
    v = v.replace("nvme ssd", "nvme")
    v = v.replace("nvme-ssd", "nvme")
    v = v.replace("sata ssd", "sata")

    v = v.replace("small form factor", "sff")
    v = v.replace("small-form-factor", "sff")
    v = v.replace("small formfactor", "sff")
    v = v.replace("keyboard & mouse", "keyboard mouse")
    v = v.replace("keyboard and mouse", "keyboard mouse")
    v = v.replace("usb wired", "wired")
    v = v.replace("m dot 2", "m2")
    v = v.replace("m.2", "m2")
    v = v.replace("pcie", "pci")
    v = v.replace("pci express", "pci")
...
def _match_is_blank(value):
    raw = _raw_text(value)
    return raw in {"", "na", "n a", "null", "select", "no data"}
...
def _extract_feature_from_label_value(label, value):
    ...
    elif "usb type a port version 2" in label_clean or "usb 2" in label_clean:
        mapping["usb2"] = num
    elif "usb type a port version 3" in label_clean or "usb 3" in label_clean:
        mapping["usb3"] = num
...
def _motherboard_match_50_percent(bid_value, product, catalogue_keys):
    ...
    for key, bid_val in bid_features.items():
        # Bid dropdown me jo feature mentioned nahi hai usko ignore karo.
        if bid_val == 0:
            continue
    ...
    return percent >= 50, "Motherboard + Ports 50% Rule", " | ".join(details), percent
...
def _monitor_size_match(bid_value, product, catalogue_keys):
    ...
    for screen_value in screen_values:
        ...
        # ±1 inch tolerance. Isse 19 inch aur 19.5 inch dono accept honge.
...
def _special_zero_field_match(bid_key, bid_value, product, catalogue_keys):
    ...
    if bid_key == "wifi":
        # Form me WiFi None hai, catalogue me M.2 WiFi slot available = 1.
        for key, val in pairs:
...
def match_catalogue_models(request, bid_id):
    ...
    bid_specs = {
        "processor":   (getattr(bid, "processor", "") or "").strip(),
        "ram":         (getattr(bid, "ram", "") or "").strip(),
        "hdd":         (getattr(bid, "hdd", "") or "").strip(),

        # Sirf SSD1 match hoga. SSD2 ignore.
        "ssd":         (getattr(bid, "ssd1", "") or getattr(bid, "ssd", "") or "").strip(),

        "os":          (getattr(bid, "os", "") or "").strip(),
        "dvd":         (getattr(bid, "dvd", "") or "").strip(),
        "wifi":        (getattr(bid, "wifi", "") or "").strip(),
        "motherboard": (getattr(bid, "motherboard", "") or "").strip(),
        "monitor":     (getattr(bid, "monitor", "") or "").strip(),
        "cabinet":     (getattr(bid, "cabinet", "") or "").strip(),
        "keyboard":    (getattr(bid, "keyboard", "") or "").strip(),
        "warranty":    (getattr(bid, "warranty", "") or "").strip(),
    }
    ...
    perfect_results = [
        r for r in results
        if r["total_checked"] == MATCH_REQUIRED_FIELDS
        and r["match_count"] == MATCH_REQUIRED_FIELDS
    ]
    ...
    if not perfect_results:
        return JsonResponse({
            ...
            "message": "12/12 matching model nahi mila.",
        }, status=200)

    perfect_results.sort(
        key=lambda x: (
            -x["match_count"],
            -x["total_score"],
            x["model_no"],
    ...
    try:
        ...
        for index, month_name in enumerate(months, start=1):
            result.append({
                ...
                "monthNumber": index,
                "total": 0,
                "pending": 0,    # analyser submitted, admin pending
                "approved": 0,   # admin approved
                "rejected": 0,   # admin re-analyze (rejected)
            })
