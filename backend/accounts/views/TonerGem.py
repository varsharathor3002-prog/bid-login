"""Toner's GeM-upload job queue — structurally the same as AIO's (AioGem.py),
scoped to TonerBid instead of AioBid.

Scope note: same limitation as AIO's own `_aio_gem_payload` — this gives the
Analyser/Admin a working "Upload to GeM" button (opens the real GeM login
page and tracks the upload's status) and a job-queue/audit trail identical in
shape, but does NOT reproduce Desktop's extension auto-fill spec mapping.
Toner's real GeM category ("Toner Cartridge") has its own form nobody has
captured here, so `_toner_gem_payload` only fills in values genuinely known
from the bid itself.
"""
import re
...
def _toner_gem_payload(bid, request):
    ...
    return {
        ...
        "specifications": {
            **catalogue_specs,
            "Brand": bid.brand or "",
            "Cartridge Type": bid.cartridge_type or "",
            "Product Class": bid.product_class or "",
            "Colour": bid.colour or "",
            "Technology": bid.technology or "",
            "Compatibility": bid.compatibility or "",
            "Page Yield": bid.page_yield or "",
            "Yield Standard": bid.yield_standard or "",
            "Chip": bid.chip or "",
            "Print Coverage": bid.print_coverage or "",
            "Warranty": bid.warranty or "",
            "Warranty Type": bid.warranty_type or "",
            "Refillable": bid.refillable or "",
        },
        "documents": [
            {
                "type": doc_type,
                "url": request.build_absolute_uri(f"/api/toner-bids/{bid.id}/generate-docs/"),
            }
...
def create_toner_gem_upload_job(request, bid_id):
    data = _json_body(request)
    user, error = _require_role(request, {"analyser"}, data)
    if error:
        return error
    _ensure_table();_ensure_gem_tables()
    bid = TonerBid.objects.filter(id=bid_id).first()
    if not bid:
        return JsonResponse({"error": "Toner bid not found."}, status=404)
    if bid.status != "approved":
        return JsonResponse({"error": "Admin approval is required before GeM upload."}, status=409)
    payload = _toner_gem_payload(bid, request)
...
def toner_gem_jobs(request):
    user, error = _require_role(request, {"admin", "analyser"})
    if error:
        return error
    _ensure_gem_tables()
    jobs = TonerGemUploadJob.objects.select_related("bid", "triggered_by")
...
@csrf_exempt
@require_http_methods(["GET"])
def toner_gem_job_detail(request, job_id):
    user, error = _require_role(request, {"admin", "analyser"})
    if error:
        return error
    _ensure_gem_tables()
    job = TonerGemUploadJob.objects.select_related("bid", "triggered_by").filter(id=job_id).first()
    if not job:
...
@csrf_exempt
@require_http_methods(["GET"])
def extension_toner_jobs(request):
    user, error = _require_role(request, {"analyser", "admin"})
    if error:
        return error
    _ensure_gem_tables()
    jobs = TonerGemUploadJob.objects.select_related("bid", "triggered_by").filter(
    ...
    jobs = list(jobs[:25])
    for job in jobs:
        payload = _toner_gem_payload(job.bid, request)
...
@csrf_exempt
@require_http_methods(["POST"])
def extension_toner_claim_job(request, job_id):
    data = _json_body(request)
    ...
    TonerGemAuditLog.objects.create(job=job, actor=user, event="extension_claimed", message="Chrome extension claimed the job.")
    return JsonResponse({**_job_data(job), "payload": job.payload_snapshot})


@csrf_exempt
@require_http_methods(["POST"])
def extension_toner_report_job(request, job_id):
    data = _json_body(request)
    user, error = _require_role(request, {"analyser", "admin"}, data)
    if error:
        return error
...
@csrf_exempt
@require_http_methods(["POST"])
def retry_toner_gem_job(request, job_id):
    data = _json_body(request)
    user, error = _require_role(request, {"admin"}, data)
    if error:
        return error
    _ensure_gem_tables()
    job = TonerGemUploadJob.objects.filter(id=job_id).first()
    if not job:
        return JsonResponse({"error": "Upload job not found."}, status=404)
    ...
    TonerGemAuditLog.objects.create(job=job, actor=user, event="job_retried", message="Job manually retried.")
    return JsonResponse(_job_data(job))


@csrf_exempt
@require_http_methods(["PATCH", "POST"])
def update_toner_gem_status(request, bid_id):
    """Same-shaped callback endpoint as AIO's update_aio_gem_status, for a
    direct (non-job-queue) status ping from the callback_url in the payload above."""
    try:
        bid = TonerBid.objects.get(id=bid_id)
    except TonerBid.DoesNotExist:
        return JsonResponse({"error": "Toner bid not found."}, status=404)
    data = _json_body(request)
